...
def _handle_command(chat, cmd, arg):
    """Handle slash commands. Returns True to continue loop, False to break."""
    if cmd == "/quit":
        print("Goodbye!")
        return False
...
def _send_and_display(chat, user_input):
...
def main():
    ...
    while True:
        ...
        if user_input.startswith("/"):
            ...
            if not _handle_command(chat, cmd, arg):
                break
            continue

        # Send message to model
        try:
            _send_and_display(chat, user_input)
        except Exception as e:
            print(f"\nError: {e}")
